...
/** Explicit export map for GAS backend services */
const EXPORT_MAP = {
  'Commons.gs': ['StickerSheetRepository'],
  'ImportExportService.gs': ['ImportExportService'],
  'QuickEntryService.gs': ['QuickEntryService']
}
...
function buildGsFile ({ relPath, fileName, content }) {
  ...
  const exportedFromSource = extractExportsFromTags(content)
  const exports = exportedFromSource.length ? exportedFromSource : (EXPORT_MAP[fileName] || [])
...
/** Extracts @export tags from helper source (fallback) */
function extractExportsFromTags (content) {
  const exported = []
  const jsdocExportRegex =
    /\/\*\*[\s\S]*?@export[\s\S]*?\*\/\s*(async\s+)?function\s+([a-zA-Z0-9_]+)/g
  let match

  while ((match = jsdocExportRegex.exec(content)) !== null) {
