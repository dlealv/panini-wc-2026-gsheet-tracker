...
1. All whitespace and non-ASCII characters are stripped (flag emojis are non-ASCII and are removed here, regardless of where they appear in the line).
2. The delimiter is normalized to `,`.
3. Country codes are normalized to uppercase.
4. All possible repeat representations (`NxX`, `N(xX)`, `A-BxX`, `A-B(xX)`) are normalized to the canonical repeat forms: `N(X)`, `A-B(X)`.
...
- Accepted delimiters between tokens are `,` (comma) and `;` (semicolon); both are normalized to `,` before parsing.
