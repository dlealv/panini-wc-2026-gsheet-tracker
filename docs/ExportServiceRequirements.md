...
Each section begins with a header line. The header includes, in parentheses, the total count of distinct sticker numbers in that section across all countries (not a line count, and not counting repeat copies). The header is part of the exported text and is included when copying or downloading the export.
...
In this example, the repeats header shows `(15)` because the three countries together list 5 + 5 + 5 = 15 distinct repeated sticker numbers; the missing header shows `(15)` because they together list 6 + 4 + 5 = 15 distinct missing sticker numbers.
...
- Header of the section: `🔄 Repeats (N)`, where `N` is the total count of distinct repeated sticker numbers across all countries (e.g. `🔄 Repeats (15)`).
...
- Header of the section: `❌ Missing (N)`, where `N` is the total count of distinct missing sticker numbers across all countries (e.g. `❌ Missing (15)`).
